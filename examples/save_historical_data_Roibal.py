...
def save_historic_klines_csv(symbol, start, end, interval):
    ...
    mpl_finance.candlestick_ochl(ax, ochl, width=2)
    print(three_period_moving_ave)
    print(len(three_period_moving_ave))
    ...
    plt.legend()
    plt.show()
def avg_3(symbol, start, end, interval):
    klines = get_historical_klines(symbol, interval, start, end)
    ochl = []
    list_of_open = []
    three_period_moving_ave = []
    ...
    with open('Binance_{}_{}-{}.txt'.format(symbol, start, end), 'w') as f:
        ...
        for kline in klines:
            ...
            if len(list_of_open)>9:
                ...
                ten_period_moving_ave.append(float(price10/10))
                time10.append(time1)
    return three_period_moving_ave
def candles(symbol, start, end, interval):
    klines = get_historical_klines(symbol, interval, start, end)
    ochl = []
    list_of_open = []
    three_period_moving_ave = []
